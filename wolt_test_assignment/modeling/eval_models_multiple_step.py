...
from wolt_test_assignment.config import FIGURES_DIR, INTERIM_DATA_DIR, MODELS_DIR, PROCESSED_DATA_DIR, SPLIT_DATE
...
def eval_LR_model(
    ...
    start_date = datetime.strptime(SPLIT_DATE, "%Y-%m-%d") + timedelta(days=training_days + start_ind)
...
def eval_LSTM_model(
    ...
    X_total = []
    for i in range(training_days, len(total_set_scaled) - n_steps + 1):
        X_total.append(total_set_scaled[i - training_days:i])  # Flatten for MultiOutputRegressor input    
    ...
    predicted_courier_number = model.predict(X_total, verbose = 0)[len_train:]  # Predictions for the test set
    ...
    # Extract the original scale predictions for 'courier_partners_online'
    predicted_courier_number_original = predicted_original[:, 0]
    integer_predicted_courier_number = np.round(predicted_courier_number_original).astype(int)
...
def main(
    ...
    for start_ind in range(0, number_intervals):
        ...
        y_test_original, predicted_courier_number_original, start_date = eval_LSTM_model(model_path, training_days, n_steps, start_ind) 
        ...
        if start_ind % 10 == 0:
            ...
            plot_prediction(y_test_original, predicted_courier_number_original, start_date, 
                        figure_title = f"LSTM prediction {n_steps} days forward", 
                        figure_path = FIGURES_DIR / f"plot_LSTM_featureDays_{training_days}_steps_{n_steps}_days_interval_{start_ind}.png")            
    ...
    for start_ind in range(0, number_intervals):
        ...
        y_test_original, predicted_courier_number_original, start_date_str = eval_LR_model(model_path, training_days, n_steps, start_ind) 
        ...
        if start_ind % 10 == 0:
            ...
            plot_prediction(y_test_original, predicted_courier_number_original, start_date_str,
                        ...
                        figure_path = FIGURES_DIR / f"plot_LR_featureDays_{training_days}_steps_{n_steps}_days_interval_{start_ind}.png")
